...
By default this uses SQLite (no setup needed). To use MySQL instead:
1. In `backend/requirements.txt`, uncomment `mysqlclient` and reinstall
2. In `backend/.env`, set `DB_ENGINE=mysql` and fill in your DB credentials
3. Create the database, then run `python manage.py migrate` again